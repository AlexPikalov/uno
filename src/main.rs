...
use lang::ast::{expr_parser, Func, Statement};
...
"#;
    ...
    let main_func_decl = module.iter().find(|stmt| match stmt {
        Statement::FuncDecl(Func { name, .. }) => name == "main",
        _ => false,
    });
    println!("main fn: \n {main_func_decl:#?}");
